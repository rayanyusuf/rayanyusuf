...
export type AdminWorkspaceVariant = "papers" | "answers";

export function AdminPastPapersWorkspace({
  variant,
}: {
  variant: AdminWorkspaceVariant;
}) {
  const router = useRouter();
  const isAnswers = variant === "answers";
  const uploadsDir = isAnswers ? "answers/uploads" : "uploads";
  const pagesRoot = isAnswers ? "answers/pages" : "pages";
  const problemsDir = isAnswers ? "answers/problems" : "problems";
  const confirmedDir = isAnswers ? "answers/confirmed" : "confirmed";
  const problemsTable = isAnswers ? "answers" : "problems";
  const idCol = isAnswers ? "answer_id" : "problem_id";
   ...
   * - answer id: same question key as problems when PDF is Further-Maths-YEAR-Paper-N-Answers.pdf
   */
  ...
  /**
   * For Answers tab: prefer same paperId as past papers so answer rows match `problems` IDs
   * (Further-Maths-YEAR-paper-N). Parses Further-Maths-YEAR-Paper-N-Answers from filename, else CP / legacy Answers-N.
   */
  const getAnswerMeta = (paperName: string) => {
    const base = paperName.replace(/\.pdf$/i, "").trim();
    const yearMatch = base.match(/\b(20\d{2})\b/);
    const year = yearMatch ? yearMatch[1]! : new Date().getFullYear().toString();
    const paperFromAnswersSuffix = base.match(/\bPaper\s*(\d+)\s*-?\s*Answers\b/i);
    if (paperFromAnswersSuffix) {
      const paperNum = paperFromAnswersSuffix[1] ?? "1";
      const paperId = `Further-Maths-${year}-paper-${paperNum}`;
      return { year, answersNum: paperNum, paperId };
    }
    const answersMatch = base.match(/\bAnswers-(\d+)\b/i) ?? base.match(/\bCP\s*(\d+)\b/i);
    const n = answersMatch ? (answersMatch[1] ?? "1") : "1";
    const paperId = `Further-Maths-${year}-Answers-${n}`;
    return { year, answersNum: n, paperId };
  };

  const getPaperId = (paperName: string) =>
    isAnswers ? getAnswerMeta(paperName).paperId : getPaperMeta(paperName).paperId;
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase.storage
        .from(PAST_PAPERS_BUCKET)
        .list(uploadsDir, { limit: 100, offset: 0, sortBy: { column: "name", order: "desc" } });

      if (error) return;
      const pdfs =
        data
          ?.filter((o) => o.name.toLowerCase().endsWith(".pdf"))
          .map((o) => ({ name: o.name, path: `${uploadsDir}/${o.name}` })) ?? [];
  ...
  useEffect(() => {
    ...
    const safePaperBase = paperBase
      ...
      .replace(/_+/g, "_")
      .replace(/^_+|_+$/g, "");
    const paperId = getPaperId(selectedPaper.name);
    ...
    const loadPages = async () => {
      const tryFolders = [`${pagesRoot}/${safePaperId}`, `${pagesRoot}/${safePaperBase}`];
      ...
      for (const folder of tryFolders) {
        ...
        if (res.data && res.data.length > 0) {
          ...
          chosenFolderBase = folder.startsWith(`${pagesRoot}/`)
            ? folder.slice(pagesRoot.length + 1)
            : folder.replace(/^pages\//, "");
      ...
      const parsed = (await Promise.all(
        pngs.map(async (o) => {
          ...
          const path = `${pagesRoot}/${folderBase}/${o.name}`;
  ...
  useEffect(() => {
    ...
    const loadConfirmed = async () => {
      ...
      const rows = (data as Record<string, string>[] | null | undefined) ?? [];
      const ids = new Set<string>(rows.map((r) => r[idCol] as string));
      setConfirmedProblemIds(ids);
    };

    loadConfirmed();
  }, [selectedPaper, problemsTable, idCol]);
  ...
  const uploadPdfs = async (files: File[]) => {
    ...
    try {
      ...
      >(
        pdfs.map(async (file) => {
          // Naming rules:
          // - Papers:  Further-Maths-YEAR-Paper-N.pdf
          // - Answers: Further-Maths-YEAR-Paper-N-Answers.pdf
          // For answers, YEAR comes from filename and N comes from AI scan of page 1.
          let standardName: string | null = null;
          const base = file.name.replace(/\.pdf$/i, "").trim();
          const yearMatch = base.match(/\b(20\d{2})\b/);
          const yearFromFile = yearMatch ? Number(yearMatch[1]) : NaN;

          try {
            if (!isAnswers) {
              // Fast path for papers if filename already has "Paper N".
              const paperMatch = base.match(/\bPaper\s*(\d+)\b/i);
              const paper = paperMatch ? Number(paperMatch[1]) : 1;
              if (Number.isFinite(yearFromFile)) {
                standardName = `Further-Maths-${yearFromFile}-Paper-${paper}.pdf`;
              }
            }

            // For answers (always), or papers fallback, render page 1 and ask AI.
            if (isAnswers || !standardName) {
              ...
              if (res.ok && json.paper) {
                if (isAnswers) {
                  if (Number.isFinite(yearFromFile)) {
                    standardName = `Further-Maths-${yearFromFile}-Paper-${json.paper}-Answers.pdf`;
                  } else {
                    setUploadError(
                      (prev) =>
                        prev ??
                        `Solution key naming needs a YEAR in the filename for ${file.name}.`
                    );
                  }
                } else if (json.year) {
  ...
  const deletePdf = async (p: { path: string; name: string }) => {
    ...
    try {
      ...
      const legacyBase = paperBase
        ...
        .replace(/_+/g, "_")
        .replace(/^_+|_+$/g, "");
      const paperId = getPaperId(p.name)
        .trim()
        .replace(/[^\w.\-]+/g, "_")
        .replace(/_+/g, "_")
        .replace(/^_+|_+$/g, "");

      const folders = [`${pagesRoot}/${paperId}`, `${pagesRoot}/${legacyBase}`];
  ...
  const convertSelectedPdfToPngs = async () => {
    ...
    try {
      ...
      for (let pageNumber = 1; pageNumber <= totalPages; pageNumber++) {
        ...
        const path = `${pagesRoot}/${safePaperBase}/page-${String(pageNumber).padStart(3, "0")}.png`;
  ...
  const confirmCrop = async () => {
    ...
    try {
      ...
      const safePaper = paperBase
        ...
        .replace(/_+/g, "_")
        .replace(/^_+|_+$/g, "");
      const path = `${problemsDir}/${safePaper}/page-${String(selectedPage.pageNumber).padStart(
        3,
        "0"
  ...
  /** Standardized id: Further-Maths-<year>-paper-<n>-Question-<q> (or -Answers-<n>- for answers tab). */
  const generateProblemId = (paperName: string, pageNumber: number): string => {
    const paperId = getPaperId(paperName);
  ...
  const confirmProblem = async () => {
    ...
    try {
      ...
      const problemId = `${getPaperId(selectedPaper.name)}-Question-${qJson.question}`;
      if (confirmedProblemIds.has(problemId)) {
        throw new Error(`Already saved: ${problemId}`);
      }
      const confirmedPath = `${confirmedDir}/${problemId}.png`;
      ...
      const { error: dbErr } = await supabase.from(problemsTable).upsert(
        isAnswers
          ? { answer_id: problemId, answer_image: confirmedPath }
        ...
        <div className="flex-1 min-w-0 flex flex-col rounded-xl border border-zinc-800 bg-zinc-950/60 p-4 relative overflow-hidden">
          ...
          {pages.length > 0 && selectedPage?.url ? (
              ...
              {extracted.length > 0 && (
                <div className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900/50 p-2">
                  <div className="text-xs font-semibold text-zinc-400 mb-2">
                    {isAnswers ? "Extracted answer" : "Extracted problem"}
                  </div>
            ...
            <div className="flex-1 flex items-center justify-center text-zinc-500 text-sm">
              {isAnswers
                ? "Drop a solution key PDF on the left, convert it, then select a page to crop and extract answers."
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-emerald-500/10 border-2 border-emerald-400 rounded-xl z-10">
              <div className="text-center">
                ...
                <div className="mt-1 text-sm text-zinc-300">
                  {isAnswers ? "Upload solution key PDFs" : "Upload past papers"}
                </div>
              </div>
            </div>
          )}
